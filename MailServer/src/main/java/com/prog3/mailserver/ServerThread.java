...
public class ServerThread extends Thread {
    ...
    private Object handleRequest(Object request) throws IOException {
        if (request instanceof LoginRequest) { // CASO 1: Login di un client
            return getMailBox((LoginRequest) request);
        } else if (request instanceof SendEmailRequest) { // CASO 2: Invio email da parte di un client
            return sendEmail((SendEmailRequest) request);
        } else if (request instanceof ReceivedEmailRequest) { // CASO3: Ricezione email da parte di un client
            return receivedEmail((ReceivedEmailRequest) request);
        } else if (request instanceof DeleteEmailRequest) { // CASO 4: Eliminazione di un'email da parte di un client
            return deleteEmail((DeleteEmailRequest) request);
        }  else if (request instanceof CheckNewEmailsRequest) { // CASO 5: Verifica presenza nuove email
            return getNewEmails((CheckNewEmailsRequest) request);
        } else if (request instanceof String) { // CASO 6: Client disconnesso
            controllerPrimary.printLog("Connection closed for " + (String) request);
        }
        return null;
    }

    private MailBox getMailBox(LoginRequest request) { // Stampa il log di connessione dell'account che si è collegato
    ...
    private boolean sendEmail(SendEmailRequest request) { // Stampa il log di invio email dell'account
        ...
        // Trova tutti i destinatari dell'e-mail
        List<MailBox> recipients = findRecipients(email.getReceiver());
        // Trova i destinatari errati (non esistenti) dell'e-mail
        ...
        if (!recipients.isEmpty() || !nonExistingRecipients.isEmpty()) {
            // Per ogni email inesistente inviare al mittente una email di errore
            for (String e : nonExistingRecipients){
                ...
                // Scrivi l'e-mail di errore nel file
                try {
                ...
                // Aggiungi l'e-mail di errore alla mailBox
                for(int i = 0; i < accounts.size(); i++) {
                    if(accounts.get(i).getEmail().equals(email.getSender())){
                        synchronized (accounts.get(i)){ // Solo un thread alla volta aggiunge una email contemporanemente
            ...
            for (MailBox account : accounts) {
                if (account.getEmail().equals(email.getSender())) {
                    ...
                    if(receivers!=null && receivers.size()>=1) // Se vi sono uno o più destinatari esistenti
            ...
            // Aggiungi l'email alla mailbox di ogni destinatariuo
    ...
    private Object deleteEmail(DeleteEmailRequest request) {
        ...
        for (MailBox account : accounts) { // Ciclo gli account
            if (account.getEmail().equals(email)) { // Trovo l'account nella quale voglio effettuare la delete
                for (int i = 0; i < account.getListReceivedEmails().size(); i++) { // Cerco la email nella lista di email
                    if (account.getListReceivedEmails().get(i).getID() == id) { // Appena trovo l'email
                        account.getListReceivedEmails().remove(i); // La elimino
                    }
                }
    ...
    private List<Email> getNewEmails(CheckNewEmailsRequest request) {
        ...
        for (MailBox account : accounts) { // Scorro gli account
            synchronized (account){
                if (account.getEmail().equals(email)) { // Trovo l'account corretto
                    boolean x = false;
                    List<Email> newEmails = new ArrayList<>();
                    for (int i = 0; i < account.getListReceivedEmails().size(); i++) {
                        if (account.getListReceivedEmails().get(i).getID() > lastEmailId) { // Vi è una nuova email se c'è un ID maggiore dell'ultimo
