...
public class Server {
     ...
     */
    private void readFile() { // Legge le email
        ...
        try (InputStreamReader isr = new InputStreamReader(ioStream); BufferedReader br = new BufferedReader(isr);) {
            ...
            while ((line = br.readLine()) != null) {
                String name = line.substring(0, line.indexOf(",")); // Nome
                String email = line.substring(line.indexOf(",") + 1); // Email
    ...
    public void deleteEmail(String email, int id) {
        ...
        synchronized (fileNames.get(i)){
            try {
                ...
                BufferedReader reader = new BufferedReader(new FileReader(emailFile));
                String line = reader.readLine();
                while (line != null) { // Cerca la linea della email da eliminare
                    Email e = Email.fromString(line);
                    if (e.getID() != id) {
                        lines.add(line);
                    }
    ...
    synchronized public void activate() throws IOException {
        readFile();
        readIncomingEmail();
        new Thread(() -> { // Questo thread rimane sempre in ascolto (e aspetta la connessione di uno o più client)
            try {
